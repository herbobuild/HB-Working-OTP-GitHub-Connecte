...
class Megamenu {
  ...
  initProductsBanner(banner) {
    ...
    let slider;
    loadAssets(['swiper.aio.min.css', 'swiper.aio.min.js']).then(() => {
...
register('video-section', {
  onLoad: function () {
    const initVideo = () => {
      loadAssets(['plyr.aio.min.css', 'plyr.aio.min.js']).then(() => {
...
register('collection-template', {
  onLoad: () => {
    loadAssets('collection.aio.min.js').then(() => collection_MinimogTheme.Collection.init()).catch(console.error);
  }
});
...
class ProductHelper {
  ...
  constructor(_ref) {
    ...
    _defineProperty(this, "initLightbox", () => {
      ...
      loadAssets(['glightbox.aio.min.css', 'glightbox.aio.min.js']).then(() => {
